...
| Hành động | Ở đâu |
|-----------|-------|
| Đăng ký package | Consumer Portal → Plans |
| Yêu cầu truy cập extension | Marketplace → Chi tiết Extension → Request Subscription |
| Xem trạng thái đăng ký | Marketplace → My Subscriptions |
| Xem sử dụng | Consumer Portal → Usage Analytics |
| Quản lý API key | Consumer Portal → API Keys |
| Xem hóa đơn | Consumer Portal → Billing |
